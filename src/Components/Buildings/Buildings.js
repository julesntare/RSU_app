            ...
            <div   className="my-2  bg-none buildings-box  pt-3 row mt-2">
                {
                    BuildingsList.map((item, i)=>{
                        return(
                            <div className=" card border building-card m-1 p-0 col-12 col-md-4 col-lg-3" key={item.id}>
                                    <img className="card-img-top m-0" src={BuildingImg} alt="Card image cap"></img>
                                    <p className="text-center card-name-title my-2"> {item.name.toUpperCase() }{ item.other_names?  <span className="ms-1 text-sm ">( {item.other_names} )</span>: ""}</p>
                                    <div className="card-body p-2">
                                          ...
                                          <div className="py-1 d-flex flex-column justify-content-center btn-box">
                                                <button className=" d-flex justify-content-center bg-primary text-white fw-bold btn-sm align-items-center rounded-pill "
